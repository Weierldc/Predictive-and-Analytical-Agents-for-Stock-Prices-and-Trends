...
class TransformerEncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):  # 添加training默认参数
        # 多头注意力
...
# --- 2. 创建Transformer模型构建函数 (修改为多输出) ---
def build_transformer_model(input_shape, num_layers=2, d_model=64, num_heads=4, dff=128, dropout_rate=0.1):
    """
    构建Transformer模型用于时间序列预测
    ...
    # 创建模型
    model = Model(inputs=inputs, outputs=outputs, name=f"Transformer_l{num_layers}_d{d_model}_h{num_heads}")

    return model


# --- 3. Transformer模型训练与评估函数 ---
def train_transformer_model(X_train, y_train, X_val, y_val, transformer_params, industry, ticker, epochs=100,
                            batch_size=32):
    """
    训练Transformer模型并评估
    ...
    Args:
        ...
        X_val, y_val: 验证数据
        transformer_params: Transformer模型参数字典
    ...
    dff = transformer_params.get('dff', 128)
    ...
    # 设置回调
    checkpoint_dir = 'model_checkpoints_transformer'
    os.makedirs(checkpoint_dir, exist_ok=True)

    model_id = f"transformer_l{num_layers}_d{d_model}_h{num_heads}"
    filepath = os.path.join(checkpoint_dir, f'transformer_model-{model_id}-{{epoch:02d}}-{{val_mape:.2f}}.keras')
    ...
    final_model_path = os.path.join(model_dir, f'final_transformer_{englishname(industry)}.keras')
...
# --- 4. Transformer预测函数 (适配多输出) ---
def predict_with_transformer(model, X_test, scaler=None, actual_values=None):
    """
...
# --- 5. Transformer可视化函数 (适配多输出) ---
def visualize_transformer_predictions(actual_values, predictions, ticker, title=None):
    """
    可视化Transformer预测结果
    ...
    for day in range(actual_values.shape[1]):
        ...
        plt.plot(x, predictions[:, day], 'm--', label='预测价格', linewidth=1.5)  # 品红色线条
        ...
        plt.figtext(0.15 + 0.3 * (day % 3), 0.45 - 0.45 * (day // 3), f'MAPE: {day_mape:.2f}%',
                    fontsize=10, bbox={'facecolor': 'violet', 'alpha': 0.1, 'pad': 3})

    # 添加主标题
    if title:
        plt.suptitle(f"{title}\n整体MAPE: {mape:.2f}%", fontsize=16)
    else:
        plt.suptitle(f'{ticker} 股票价格 Transformer 预测\n整体MAPE: {mape:.2f}%', fontsize=16)
    ...
    output_path = os.path.join(output_dir, f'transformer_prediction_{ticker}.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()


# --- 6. Transformer训练历史可视化函数 ---
def plot_transformer_training_history(history):
    ...
    output_dir = "picture"
    os.makedirs(output_dir, exist_ok=True)  # 创建文件夹（如果不存在）
    output_path = os.path.join(output_dir, f'transformer_training_history.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def visualize_future_predictions(history_df, future_prices, ticker, industry):
    ...
    # 绘制预测价格
    plt.plot(future_dates, future_prices, 'mo-', label='预测价格', linewidth=2, markersize=8)  # 品红色线条
    ...
    plt.fill_between(future_dates, lower_bound, upper_bound,
                     color='violet', alpha=0.1, label='置信区间')
...
# --- 7. 实施Transformer模型训练主函数 (修改未来预测逻辑) ---
def train_stock_transformer_model(ticker, industry, transformer_params=None, test_size=0.2, start_date=None,
                                  end_date=None):
    """
    ...
    Args:
        ticker: 股票代码
        industry: 行业名称
        transformer_params: Transformer模型参数
        test_size: 测试集比例
        start_date: 开始日期
        end_date: 结束日期
    ...
    """
    print(f"\n{'=' * 50}")
    print(f"开始Transformer模型训练流程: {ticker} ({industry}行业)")
    print(f"{'=' * 50}")
    ...
    X, y, X_lately, scaler, feature_list = enhanced_stock_data_preprocessing(
        df_original_data,
        transformer_params['mem_days'],
        transformer_params['pred_days']
    )
    ...
    # 4. 训练模型
    print(f"\n4. 训练Transformer模型...")
    model, history, model_path = train_transformer_model(
        X_train, y_train,
        X_test, y_test,
        transformer_params,
        industry,
        ticker,
        epochs=100,  # Transformer通常需要更多epochs
        batch_size=32
    )
    ...
    # 6. 在测试集上预测
    print(f"\n6. 在测试集上进行预测...")
    predictions, metrics = predict_with_transformer(model, X_test, scaler, y_test)

    # 7. 可视化预测结果
    print(f"\n7. 可视化预测结果...")
    visualize_transformer_predictions(y_test, predictions, ticker, f"{ticker} ({industry}行业) Transformer预测结果")

    # 8. 保存scaler和特征列表
    model_dir = 'models'
    os.makedirs(model_dir, exist_ok=True)

    scaler_path = os.path.join(model_dir, f'transformer_scaler_{englishname(industry)}.pkl')
    joblib.dump(scaler, scaler_path)
    print(f"Scaler已保存至: {scaler_path}")

    feature_list_path = os.path.join(model_dir, f'transformer_features_{englishname(industry)}.pkl')
    joblib.dump(feature_list, feature_list_path)
    print(f"特征列表已保存至: {feature_list_path}")

    # 9. 预测未来价格
    print(f"\n9. 预测未来{transformer_params['pred_days']}天价格...")
    if X_lately is not None and len(X_lately) > 0:
        ...
        print("   没有最新数据用于预测")
        future_predictions = np.zeros(transformer_params['pred_days'])
    ...
    print(f"\n{'=' * 50}")
    print(f"Transformer模型训练完成: {ticker} ({industry}行业)")
    print(f"{'=' * 50}")
...
def main():
    ...
    # 训练模型
    model, scaler, feature_list = train_stock_transformer_model(
        ticker,
        industry,
        transformer_params,
        start_date=start,
