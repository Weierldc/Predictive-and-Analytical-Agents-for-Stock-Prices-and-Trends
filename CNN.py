...
def weighted_mse_loss(y_true, y_pred):
    """
    加权MSE损失函数，近期预测误差权重更高
    权重分配：最近一天权重=5，最远一天权重=1
    """
    weights = tf.linspace(5.0, 1.0, tf.shape(y_pred)[1])
    squared_diff = tf.square(y_true - y_pred)
    weighted_loss = tf.reduce_mean(squared_diff * weights, axis=-1)
    return weighted_loss


# --- 创建专门的CNN模型构建函数 (修改为多输出) ---
def build_cnn_model(input_shape, pred_days, conv_layers=2, dense_layers=2, filters=64, kernel_size=3, pool_size=2,
                    dropout_rate=0.2):
    """
    构建CNN模型用于时间序列预测
    ...
    Returns:
        构建好的Keras模型
    """
    model = Sequential(name=f"CNN_Model_c{conv_layers}_d{dense_layers}_f{filters}")
    ...
    # 输出层 - 预测未来pred_days天的价格
    model.add(Dense(pred_days, name='output'))

    return model


# --- CNN模型训练与评估函数 (使用加权损失) ---
def train_cnn_model(X_train, y_train, X_val, y_val, cnn_params, industry, ticker, epochs=50, batch_size=32):
    """
    训练CNN模型并评估
    ...
    pred_days = cnn_params.get('pred_days', 5)
    conv_layers = cnn_params.get('conv_layers', 2)
    dense_layers = cnn_params.get('dense_layers', 2)
    filters = cnn_params.get('filters', 64)
    ...
    model.compile(
        optimizer=optimizer,
        loss=weighted_mse_loss,  # 使用加权损失函数
        metrics=['mape', 'mae']
    )
    ...
    # 设置回调
    checkpoint_dir = 'model_checkpoints_cnn'
    os.makedirs(checkpoint_dir, exist_ok=True)

    model_id = f"cnn_{conv_layers}_dense_{dense_layers}_filters_{filters}"
    filepath = os.path.join(checkpoint_dir, f'cnn_model-{model_id}-{{epoch:02d}}-{{val_mape:.2f}}.keras')
    ...
    final_model_path = os.path.join(model_dir, f'final_cnn_{englishname(industry)}.keras')
...
# --- CNN预测函数 (适配多输出) ---
def predict_with_cnn(model, X_test, scaler=None, actual_values=None):
    """
    使用训练好的CNN模型进行预测
...
# --- CNN可视化函数 (适配多输出) ---
def visualize_cnn_predictions(actual_values, predictions, ticker, title=None):
    """
    可视化CNN预测结果
    ...
    if title:
        ...
        plt.suptitle(f'{ticker} 股票价格 CNN 预测\n整体MAPE: {mape:.2f}%', fontsize=16)
    ...
    output_path = os.path.join(output_dir, f'cnn_prediction_{ticker}.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    # plt.savefig(f'cnn_prediction_{ticker}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_cnn_training_history(history):
    ...
    output_dir = "picture"
    os.makedirs(output_dir, exist_ok=True)  # 创建文件夹（如果不存在）
    output_path = os.path.join(output_dir, f'cnn_training_history.png.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
# --- 实施CNN模型训练主函数 (修改未来预测逻辑) ---
def train_stock_cnn_model(ticker, industry, cnn_params=None, test_size=0.2, start_date=None, end_date=None):
    """
    完整的CNN模型训练流程
    ...
    """
    print(f"\n{'=' * 50}")
    print(f"开始CNN模型训练流程: {ticker} ({industry}行业)")
    print(f"{'=' * 50}")
    ...
    # 4. 训练模型
    print(f"\n4. 训练CNN模型...")
    model, history, model_path = train_cnn_model(
        X_train, y_train,
        X_test, y_test,
        cnn_params,
        industry,
    ...
    # 6. 在测试集上预测
    print(f"\n6. 在测试集上进行预测...")
    predictions, metrics = predict_with_cnn(model, X_test, scaler, y_test)

    # 7. 可视化预测结果
    print(f"\n7. 可视化预测结果...")
    visualize_cnn_predictions(y_test, predictions, ticker, f"{ticker} ({industry}行业) CNN预测结果")

    # 8. 保存scaler和特征列表
    model_dir = 'models'
    os.makedirs(model_dir, exist_ok=True)

    scaler_path = os.path.join(model_dir, f'cnn_scaler_{englishname(industry)}.pkl')
    joblib.dump(scaler, scaler_path)
    print(f"Scaler已保存至: {scaler_path}")

    feature_list_path = os.path.join(model_dir, f'cnn_features_{englishname(industry)}.pkl')
    joblib.dump(feature_list, feature_list_path)
    print(f"特征列表已保存至: {feature_list_path}")

    # 9. 预测未来价格
    print(f"\n9. 预测未来{cnn_params['pred_days']}天价格...")
    ...
    print(f"\n{'=' * 50}")
    print(f"CNN模型训练完成: {ticker} ({industry}行业)")
    print(f"{'=' * 50}")
...
def main():
    ...
    cnn_params = {
        'conv_layers': 2,  # 卷积层数
        'dense_layers': 3,  # 增加全连接层数提高表达能力
        'filters': 128,  # 增加滤波器数量
        'kernel_size': 5,  # 扩大卷积核尺寸
        'pool_size': 2,
        'mem_days': 30,  # 延长历史天数
        'pred_days': 5
    }
    ...
    # 训练模型
    model, scaler, feature_list = train_stock_cnn_model(
        ticker,
        industry,
        cnn_params,
        start_date=start,
